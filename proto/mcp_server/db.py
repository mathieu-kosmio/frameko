...
def framework_criteria(slug: str) -> list[dict]:
    ...
    return query(
        ...
        "       (select max(ce.detail) from criterion_evidence ce"
        "          where ce.framework_criterion_id = fc.id) as evidence_detail,"
        "       (select jsonb_agg(distinct jsonb_build_object('slug', et.slug, 'label', et.label_fr))"
        "          from criterion_evidence ce join evidence_type et on et.id = ce.evidence_type_id"
        "          where ce.framework_criterion_id = fc.id) as evidence_types"
